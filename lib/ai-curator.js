...
function normalizeResult(raw) {
  ...
  return { suggestions, ranking, source: "gemini" };
}
...
export async function aiCuratePlaces(input, opts = {}) {
  ...
  if (!key) return { suggestions: [], ranking: {}, source: "off" };
  const model = String(opts.model ?? process.env.GEMINI_MODEL ?? DEFAULT_MODEL).trim() || DEFAULT_MODEL;
  const fetchImpl = opts.fetch ?? globalThis.fetch;
  if (typeof fetchImpl !== "function") return { suggestions: [], ranking: {}, source: "off" };
  ...
  try {
    ...
    return { suggestions: [], ranking: {}, source: "error", error: e?.message || String(e) };
  }
}
