...
// `score` opcional: cuando la IA ha recomendado un orden se pasa un scorer que
// prioriza su nota; por defecto ordena por `interestScore` como siempre.
export function topInterest(items,target,score = x => x.interestScore ?? 0) {
  return [...items].sort((a,b)=>score(b)-score(a) || (b.interestScore ?? 0)-(a.interestScore ?? 0) || String(a.name).localeCompare(String(b.name),'es') || String(a.id).localeCompare(String(b.id))).slice(0,target);
}
